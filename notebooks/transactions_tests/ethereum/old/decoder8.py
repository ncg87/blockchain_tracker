...
def normalize_hex(value):
    ...
    if isinstance(value, HexBytes):
        return value.hex() if not value.hex().startswith('0x') else value.hex()
    return value

def normalize_log(log: Dict[str, Any]) -> Dict[str, Any]:
    """
    Normalize all HexBytes values in a log dictionary to hex strings.
    """
    normalized = {}
    for key, value in log.items():
        if isinstance(value, (list, tuple)):
            normalized[key] = [normalize_hex(v) for v in value]
        else:
            normalized[key] = normalize_hex(value)
    return normalized

def normalize_logs(logs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Normalize a list of logs, converting all HexBytes to hex strings.
    """
...
class LogDecoder8:
    ...
    def decode_log(self, log: dict, event_sig: EventSignature) -> dict:
        ...
        try:
            ...
            if len(topics) < 1:
                return {"event": "Unknown", "raw_log": normalize_log(log)}
            ...
            return {
                ...
                "decode_error": str(e),
                "raw_log": normalize_log(log)
            }
    def decode_batch(self, logs_chunk: List[dict]) -> Dict[str, List[dict]]:
        """Process a batch of logs"""
        decoded = defaultdict(list)
        
        # Normalize all logs in the chunk first
        normalized_logs = normalize_logs(logs_chunk)
    ...
    def store_undecoded_log(self, log: dict, reason: str = "Unknown"):
        ...
        try:
            # Normalize the log before processing
            normalized_log = normalize_log(log)
            
            tx_hash = normalized_log.get("transactionHash")
            if not tx_hash:
                print(f"Missing transaction hash in log: {normalized_log}")
                return
                
            undecoded = UndecodedLog(
                tx_hash=tx_hash,
                log_index=normalized_log.get("logIndex", 0),
                contract=normalized_log.get("address", ""),
                signature=normalized_log["topics"][0] if normalized_log.get("topics") else "",
