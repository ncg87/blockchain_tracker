...
async def check_agent_token(token_address, db_operator):
    """
    Check if a token is an agent token by attempting to call tax-related functions.
    ...
    Returns:
        tuple: (is_agent_token, buy_tax, sell_tax)
    """
    try:
        ...
        # Check if the contract has the tax-related functions
        has_buy_tax_function = any(
            func.get('name') == 'projectBuyTaxBasisPoints' 
            for func in abi if isinstance(func, dict) and 'name' in func
        )
        
        has_sell_tax_function = any(
            func.get('name') == 'projectSellTaxBasisPoints' 
            for func in abi if isinstance(func, dict) and 'name' in func
        )
        
        if not (has_buy_tax_function and has_sell_tax_function):
            return False, None, None
        
        # Try to call the tax-related functions
        try:
            buy_tax = token_contract.functions.projectBuyTaxBasisPoints().call()
            sell_tax = token_contract.functions.projectSellTaxBasisPoints().call()
            print(f"Found agent token: {token_address} with buy tax: {buy_tax}, sell tax: {sell_tax}")
            return True, buy_tax, sell_tax
        except Exception as e:
            # Not an agent token or doesn't have these functions
            return False, None, None
...
async def export_agent_tokens(output_file="new_base_agent_tokens.csv", batch_size=50, save_interval=100):
    """
    Export all agent tokens from the Base chain to a CSV file.
    
    Args:
        output_file (str): Path to the output CSV file
        batch_size (int): Number of tokens to process in each batch
        save_interval (int): Number of tokens to process before saving progress
    """
    ...
    try:
        ...
        with open(output_file, 'a' if file_exists else 'w', newline='') as csvfile:
            ...
            if not file_exists:
                writer.writerow(['contract_address', 'is_agent_token', 'buy_tax', 'sell_tax'])
            
            # Process tokens in batches to avoid overwhelming the system
            total_tokens = len(tokens)
            agent_tokens_count = 0
            tokens_since_save = 0
            ...
            for i in range(0, total_tokens, batch_size):
                ...
                for token in batch:
                    ...
                    tasks.append(check_agent_token(token_address, db_operator))
                ...
                for j, token in enumerate(batch):
                    token_address = token['contract_address']
                    is_agent, buy_tax, sell_tax = results[j]
                    
                    writer.writerow([
                        token_address,
                        is_agent,
                        buy_tax if is_agent else '',
                        sell_tax if is_agent else ''
                    ])
                    
                    if is_agent:
                        agent_tokens_count += 1
        ...
        print(f"Found {agent_tokens_count} new agent tokens")
        print(f"Successfully exported agent token information to {output_file}")
    
    except Exception as e:
        print(f"Error exporting agent token information: {e}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Export Base chain agent tokens to CSV')
    parser.add_argument('--output', '-o', default='base_agent_tokens.csv',
                        help='Output CSV file path (default: base_agent_tokens.csv)')
    ...
    asyncio.run(export_agent_tokens(
